	...
	\item Modify Existing Alert Rule
		...
		\item City Operator
			...
			\begin{enumerate}[1.]
				...
				{\color{red}\item User selects an alert rule to modify, edits relevant environmental metrics, geographic area, threshold value(s) and timeframe, chooses whether to keep or change the alert visibility, and sends it to the administrator for approval.}
				{\color{red}\item Administrator approves the alert, the system checks if the alert is valid and saves to database.}
			...
			\begin{itemize}
				...
				\item[3ii.] User enters an invalid metric or configuration. \\
				\hspace{1cm}3ii.1. System highlights the input box in red. \\
				\hspace{1cm}3ii.2. System prevents user from proceeding until the error is corrected.
				\item[3iii.] User cancels the edit. \\
				\hspace{1cm}3iii.1. User clicks "Cancel." \\
				\hspace{1cm}3iii.2. System discards all unsaved modifications and returns the user to the "Manage Alert Rules" list.
				\item[4i.] Administrator does not approve the alert. \\
				\hspace{1cm}4i.1. Feedback box appears and admin gives feedback on alert. \\
				\hspace{1cm}4i.2. Sends feedback back to operator.
				\item[5i.] System fails to update the Alert. \\
				\hspace{1cm}5i.1. System displays a "Error" message. \\
				\hspace{1cm}5i.2. System provides an option to contact the Administrator.
			\end{itemize}
		\item Public User \\
			N/A
		\item Human Resources \\
		...
		\item Supply Chain
			\begin{itemize}
				\item[6i.] Reviews the audit log to ensure alert modifications align with current city safety protocols.
			\end{itemize}
		\item Legal
			\begin{itemize}
				\item[3i.] If an alert has been edited to be a public-facing alert, the system checks if the alert is permitted within the law.
			\end{itemize}
		\end{enumerate}
		{\bf Global Scenario:}
		\begin{itemize}
