...
This Software Requirements Specification (SRS) provides an overview of the software requirements for the Smart City Environmental Monitoring and Alert System (\textbf{SCEMAS}). \textbf{SCEMAS} is a software application designed to collect, analyze, and visualize environmental data such as air quality, temperature, humidity, and noise levels to support city-level monitoring and decision-making.
...
\\\\This document is intended for internal \textbf{SCEMAS} stakeholders, including but not limited to project managers, developers, domain experts, and \textbf{SCEMAS} team members/investors. No prior readings are required.
% End SubSection

\subsection{Scope}
\label{sub:scope}
% Begin SubSection
Smart City Environmental Monitoring and Alert System (\textbf{SCEMAS}) is an environmental monitoring 
software system that allows users to quickly and easily monitor environmental data for metrics 
such as air quality, noise levels, temperature, \textbf{UV} levels, and humidity across geographical 
...
alerts in the system. A read-only \textbf{REST API} for non-sensitive data will be publicly available 
for any user, which uses HTTP methods to ensure safe access and rate limiting to prevent abuse 
and security threats. The system will use \textbf{MQTT} for transmitting all sensor telemetry securely 
...
store or process \textbf{PII}. Audit log history of all significant events, alert triggers and user 
...
\textbf{SCEMAS} will have a separate public facing dashboard interface that will allow any public user 
to view the visualized non-sensitive data from the \textbf{REST API}.\\\\
...
accessing the \textbf{REST API} to improve data quality and product development.
% End SubSection

\subsection{Definitions, Acronyms, and Abbreviations}
\label{sub:definitions_acronyms_and_abbreviations}
% Begin SubSection
\textbf{SCEMAS}: Smart City Environmental Monitoring and Alert System\\
\textbf{PIPEDA}: Personal Information Protection and Electronic Documents Act. 
\textbf{RTO}: Recovery Time Objective is the maximum amount of time that it should take to restore system operations after a service disruption.\\
\textbf{PII}: Personally Identifiable Information\\
\textbf{UI}: User Interface is the graphical display of the application the users interact with\\
\textbf{UX}: Refers to the user experience using a given user interface\\
\textbf{MQTT}: Message Queuing Telemetry Transport\\
\textbf{RBAC}: Role-Based Access Control\\
\textbf{IoT}: Internet of Things\\
\textbf{UV}: Ultraviolet\\
\textbf{REST API}: Representational State Transfer Application Programming Interface\\
\textbf{\#-px}: Refers to the pixel size of attributes on a user interface, e.g. 14-px sized font\\
% End SubSection
...
% Begin SubSection
\textbf{SCEMAS} is a smart city monitoring dashboard developed for environmental monitoring of the 
...
various \textbf{IoT} devices based on geographic zones. However \textbf{SCEMAS} differentiates itself by 
...
The product will interact with the larger system of distributed \textbf{IoT} sensors utilizing the 
\textbf{MQTT} protocol to protect the data and ensure system privacy and security. \textbf{SCEMAS} will act 
...
to AxxonSoft map view, \textbf{SCEMAS} will overlay live heatmaps and alert markers on the geography, 
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\linewidth]{A04_System.png}
    \caption{\textbf{SCEMAS} System Diagram}
...
\begin{tabular}{ |m{2in}|m{4.5in}| } 
 ...
 \hline
 \textbf{RBAC} Management & The system supports Role-Based Access Control to differentiate administrators from city operators.  \\ 
 \hline
...
\begin{itemize}
	\item[] \textbf{Communication Protocol:} The system must accept and send all environmental telemetry data using the \textbf{MQTT} protocol. This is mandated by the existing \textbf{IoT} infrastructure already deployed in smart city environments. The system cannot use alternative protocols such as HTTP.
	\item[] \textbf{Regulation Compliance:} The system must comply with all data protection and privacy regulations, such as the \textbf{PIPEDA} in Canada. 
...
% Begin SubSection
\begin{enumerate}
	{\color{red}\item The project is assumed to be contracted to a private company to be developed. }
	...
	\item It is assumed that there is valid data flowing into the system from various \textbf{IoT} devices for Business Events to be successful (as stated in the Precondition for BE1).
...
% Begin SubSection
{\color{red}
The following features have been identified as candidates for future releases and will not be included in the first version of the system:

\begin{enumerate}
    \item \textbf{Biometrics Authentication:} Biometric login, such as fingerprint or retinal scanning, will not be included in the first version of the system and may be implemented in a future release.

    \item \textbf{Language Support:} The system will initially be developed only in English. Support for additional languages may be added in future versions of the system.

    \item \textbf{Advanced Predictive Analytics:} Predictive environmental risk analysis based on data trends will not be included in the first version of the system and may be implemented in a future release.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\linewidth]{A04_UseCase.png}
    \caption{\textbf{SCEMAS} Use-Case Diagram}
    \label{fig:use_case_diagram}
\end{figure}

The \textbf{Sensor Data} in the diagram refers to external environmental data being sent to the system.
...
\begin{itemize}
	\item {\bf BE1.} Creating an Alert Rule
	\item {\bf BE2.} Editing Existing Alert Rule
	\item {\bf BE3.} Sunlight is very high on the \textbf{UV} index
	\item {\bf BE4.} View Dashboard
	\item {\bf BE5.} Public API Request to Access Data
	\item {\bf BE6.} Checking Audit log 
	\item {\bf BE7.} User authentication
\end{itemize}

\noindent The viewpoints identified for the project include: 
\begin{itemize}
	\item {\bf VP1.} Administrator
	\item {\bf VP2.} City Operator
	\item {\bf VP3.} Public User
	\item {\bf VP4.} Human Resources
	\item {\bf VP5.} Supply Chain
	\item {\bf VP6.} Legal
\end{itemize}


\begin{enumerate}[{\bf BE1.}]
	\item Creating an Alert Rule
	\begin{itemize}
			\item[] {\bf Pre-Condition:} The user is logged in and has a valid account and is knowledgeable with the system. There is valid data flowing into the system from various \textbf{IoT} devices.
		\end{itemize}
		\begin{enumerate}[{\bf VP1.}]
		\item Administrator
			\begin{itemize}
				\item[4i.] Administrator does not approve the alert.
				\begin{itemize} 
					\item[4i.1.] Feedback box appears and admin gives feedback on alert.
					\item[4i.2.] Sends feedback back to operator.
				\end{itemize}
				\item[5i.2.] The administrator is contacted by the user about a failed alert setup.
			\end{itemize}
		\item City Operator
			\begin{itemize}
			...
			\end{itemize}
			\begin{enumerate}[1.]
				\item User clicks on create alert tool in the dashboard navbar.
				\item System displays the alert configuration tool.
				{\color{red}\item User selects relevant environmental metrics, geographic area, threshold value(s) and timeframe for this alert, selects the alert visibility and sends it to the administrator for approval.}
				{\color{red}\item Administrator approves the alert the system checks if the alert is valid and saves to database.}
				\item System creates the alert and displays a success message on screen.
			\end{enumerate}
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[1i.] User selects the wrong tool. \\
				\hspace{1cm}1i.1. User selects go back.
				\item[3i.] User enters invalid metric, or configuration. \\
				\hspace{1cm}3i.1. System highlights the input box in red. \\
				\hspace{1cm}3i.2. System prevents user from proceeding.
				\item[4i.] Administrator does not approve the alert. \\
				\hspace{1cm}4i.1. Feedback box appears and admin gives feedback on alert. \\
				\hspace{1cm}4i.2. Sends feedback back to operator.
				\item[4ii.] If a public facing alert has been created the system checks if the alert is permitted within the law.
				\item[5i.] System fails to create alert. \\
				\hspace{1cm}5i.1. System displays error message on screen. \\
				\hspace{1cm}5i.2. System provides option to contact the administrator.
			\end{itemize}
		\item Public User \\
			N/A
		...
		{\bf Global Scenario:}
		\begin{itemize}
			\item[] {\bf Precondition:} The user is logged in and has a valid account and is knowledgeable with the system. There is valid data flowing into the system from various \textbf{IoT} devices.
			\item[] {\bf Main Success Scenario:}
		\end{itemize}
		\begin{enumerate}[1.]
			\item User clicks on create alert tool in the dashboard navbar.
	...
	\item Modify Existing Alert Rule
		\begin{itemize}
			\item[] {\bf Pre-Condition:} At least one alert rule must already exist in the system database. The user must be logged in and is knowledgeable with the system and have a valid account with appropriate privileges.
		\end{itemize}
		\begin{enumerate}[{\bf VP1.}]
		\item Administrator
			\begin{itemize}
				\item[6i.] The administrator approves the alert. 
				\item[7i.] Administrator does not approve the alert.
				\begin{itemize} 
					\item[7i.1.] Feedback box appears and admin gives feedback on alert.
					\item[7i.2.] Sends feedback back to operator.
				\end{itemize}
				\item[8i.2.] The administrator is contacted by the user about a failed alert setup.
			\end{itemize}
		\item City Operator
			\begin{itemize}
				\item[] {\bf Main Success Scenario:}
			\end{itemize}
			\begin{enumerate}[1.]
				\item User selects "Manage Alerts" section.
				\item System fetches and displays a list of currently active and inactive alert rules.
				\item User selects an alert rule to modify.
				\item User edits relevant environmental metrics, geographic area, threshold value(s) and timeframe for chosen alert.
				\item User chooses whether to keep current visibility or change alert visibility, public facing vs internal.
				\item User reviews the summary and sends to administrator for approval.
				\item Once approved the system checks if the alert is valid and saves to database.
				\item System displays a success message indicating the rule has been updated.
				\item System logs the changes to Audit log.
			\end{enumerate}
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[1i.] User selects the wrong tool. \\
				\hspace{1cm}1i.1. User selects go back.
				\item[3i.] User cannot find the specific alert rule. \\
				\hspace{1cm}3i.1. User uses the search/filter bar to locate the rule by ID or name.
				\item[4i.] User enters an invalid metric or configuration. \\
				\hspace{1cm}4i.1. System highlights the input box in red. \\
				\hspace{1cm}4i.2. System prevents user from proceeding until the error is corrected.
				\item[6i.] User cancels the edit. \\
				\hspace{1cm}6i.1. User clicks "Cancel." \\
				\hspace{1cm}6i.2. System discards all unsaved modifications and returns the user to the "Manage Alert Rules" list.
				\item[7i.] Administrator does not approve the alert. \\
				\hspace{1cm}7i.1. Feedback box appears and admin gives feedback on alert.
				\item[8i.] System fails to update the Alert. \\
				\hspace{1cm}8i.1. System displays a "Error" message. \\
				\hspace{1cm}8i.2. System provides an option to contact the Administrator.
			\end{itemize}
		\item Public User \\
			N/A
		\item Human Resources \\
		...
		\item Supply Chain
			\begin{itemize}
				\item[9i.] Reviews the audit log to ensure alert modifications align with current city safety protocols.
			\end{itemize}
		\item Legal
			\begin{itemize}
				\item[5i.] If an alert has been edited to be a public-facing alert, the system checks if the alert is permitted within the law.
			\end{itemize}
		\end{enumerate}
		{\bf Global Scenario:}
		\begin{itemize}
	...
	\item Sunlight is very high on the \textbf{UV} index
		\begin{itemize}
			\item[] {\bf Pre-Condition:} \textbf{SCEMAS} is operational and is receiving sensor data. An Administrator has defined and activated an alert rule for high \textbf{UV} sunlight.
		\end{itemize}
		\begin{enumerate}[{\bf VP1.}]
		\item Administrator
			\begin{itemize}
			...
			\begin{enumerate}[1.]
				{\color{red}
					\item The system detects and logs an alert rule violation
					\item An Administrator opens alert for review
					\item The system displays triggered alert and prompts for verification
					\item Administrator verifies that the alert triggered correctly
					\item The system logs the alert verification and administrator ID to audit log
				}
			\end{enumerate}
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[2i.] System fails to generate alert. \\
				\hspace{1cm}2i.1. System logs the failure and notifies administrators.
				{\color{red}
				\item[4i.] Administrator finds fault in alert rule configuration. \\
				\hspace{1cm}4i.2. Administrator modifies the faulty alert configuration.
				}
			\end{itemize}
		\item City Operator
			\begin{itemize}
			...
			\end{itemize}
			\begin{enumerate}[1.]
				\item An alert is triggered and appears on the operator dashboard.
				\item The operator notices the alert.
				\item The operator clicks on the notification to view alert details.
				\item Alert status is updated to be acknowledged by the system.
				\item The operator and viewing timestamp is logged by the system.
			\end{enumerate}
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[1i.] Multiple alerts appear at once. \\
				\hspace{1cm}1i.1. Dashboard prioritizes the alert with higher severity. \\
				\hspace{1cm}1i.2. Continue to 2.
				\item[2i.] No operator notices the alert within 5 minutes of alert trigger. \\
				\hspace{1cm}2i.1. The system notifies administrators.
				\item[4i.] Operator flags alert as false alarm. \\
				\hspace{1cm}4i.1. System records the report and notifies administrators for review.
			\end{itemize}
		\item Public User
			\begin{itemize}
				\item[] {\bf Main Success Scenario:}
			\end{itemize}
			\begin{enumerate}[1.]
				\item The alert rule is triggered by the system.
				\item The system checks that the alert can be shown to the public.
				\item The alert is found to be publicly visible.
				\item The alert is set to be visible through the public API.
				\item A user requests access to the alert.
				\item The system sends the alert information to the user.
			\end{enumerate}
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[3i.] The alert is not found to be publicly visible. \\
				\hspace{1cm}3i.1. The alert is not set to be visible through the public API.
				\item[6i.] The system delays sending information due to rate limiting.
			\end{itemize}
		\item Human Resources \\
			N/A
		\item Supply Chain \\
		...
		\end{itemize}
		\begin{enumerate}[1.]
			\item The system detects incoming sensor data that violates an alert rule.
			\item Alert is automatically generated.
			\item Alert severity, timestamp, and affected zone are recorded.
			\item The system checks that the alert can be shown to the public.
			\item The alert is found to be publicly visible.
			\item The system notifies city operator dashboards in affected areas.
			\item The alert appears on the operator dashboard.
			\item A City Operator notices the alert.
			\item The City Operator clicks on the notification to view alert details.
			\item Alert status is updated to be acknowledged by the system.
			\item The acknowledging City Operator and timestamp is logged by the system.
			\item The alert is set to be visible through the public API.
			\item A Public User requests access to the alert.
			\item The system sends the alert information to the Public User.
			\item An Administrator reviews the triggered alert.
			\item The Administrator verifies that the alert triggered correctly.
		\end{enumerate}
		\begin{itemize}
			\item[] {\bf Secondary Scenario:}
			\item[2i.] System fails to generate alert. \\
			\hspace{1cm}2i.1. System logs the failure and notifies administrators.
			\item[5i.] The alert is not found to be publicly visible. \\
			\hspace{1cm}5i.1. The alert is not set to be visible through the public API.
			\item[7i.] Multiple alerts appear at once. \\
			\hspace{1cm}7i.1. Dashboard prioritizes the alert with higher severity. \\
			\hspace{1cm}7i.2. Continue to 8.
			\item[8i.] No operator notices the alert within 5 minutes of the alert trigger. \\
			\hspace{1cm}8i.1. The system notifies Administrators.
			\item[10i.] Operator flags alert as false alarm. \\
			\hspace{1cm}10i.1. System records the report and notifies administrators for review.
			\item[14i.] The system delays sending information due to rate limiting.
			\item[16i.] Administrator finds fault in alert rule configuration. \\
			\hspace{1cm}16i.2. Administrator modifies the faulty alert configuration.
		\end{itemize}
	...
	\item View Dashboard
		\begin{itemize}
			\item[] {\bf Pre-Condition:} \textbf{SCEMAS} is online, and the user has logged in with a verified account on \textbf{SCEMAS} with the correct level account permissions.
		\end{itemize}
		\begin{enumerate}[{\bf VP1.}]
		\item Administrator
			\begin{itemize}
			...
			\end{itemize}
			\begin{enumerate}[1.]
				\item Administrator accesses the \textbf{SCEMAS} application home screen.
				\item System retrieves the user's last-used geographical area and sets it as the current geographical location.
				\item System returns currently active nearby environmental and health warnings and displays them in a real-time dashboard map.
				\item Administrator clicks the toggle bar to select "View Alerts".
				\item System identifies the user as an Admin and loads all current environmental alerts to the dashboard.
				\item Administrator monitors active alerts and system logs for consistency.
				\item Administrator clicks log out.
				\item System invalidates the session and clears the cache.
				\item System invalidates the current session, clears cached map data and returns the user to the login screen.
			\end{enumerate}
		\item City Operator
			\begin{itemize}
				\item[] {\bf Main Success Scenario:}
			\end{itemize}
			\begin{enumerate}[1.]
				\item Operator accesses the \textbf{SCEMAS} application home screen.
				\item System retrieves the user's last-used geographical area and sets it as the current geographical location.
				\item System returns nearby currently active nearby environmental and health warnings and displays them in a real-time dashboard map.
				\item Operator clicks on a warning on the map to view more information.
				\item System opens a side panel showing all official information, including severity levels, specific metrics, and health precaution recommendations.
				\item Operator views information and closes the side panel.
				\item System closes the side panel and returns to the dashboard map.
				\item Operator selects a data type (air quality, noise levels, temperature, and humidity) from the metric toggle menu.
				\item System updates the map to display the selected data type on the real-time map with colour-coded environmental maps based on the severity levels of nearby regions.
				\item Operator clicks log out.
				\item System invalidates the current session, clears cached map data and returns the user to the login screen.
			\end{enumerate}
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[2i.] System fails to retrieve the user's last-used geographical area. \\
				\hspace{1cm}2i.1. System attempts to fetch the last-used geographical area and receives a null response from the database. \\
				\hspace{1cm}2i.2. System retrieves the user's default home location from the user's account information. \\
				\hspace{1cm}2i.3. System sets the default area as the current location and loads the map. \\
				\hspace{1cm}2i.4. Operator updates their geographical area by searching on the map.
				\item[3i.] No Critical Alerts found. \\
				\hspace{1cm}3i.1. System returns no critical alerts nearby. \\
				\hspace{1cm}3i.2. System displays a "No Active Critical Alerts" message. \\
				\hspace{1cm}3i.3. System continues to display the base map and environmental metrics side panel.
				\item[4i.] Login Timed Out. \\
				\hspace{1cm}4i.1. System notices over 15 minutes since the last user interaction. \\
				\hspace{1cm}4i.2. System clears all sensitive environmental data from the screen and clears any unsaved changes. \\
				\hspace{1cm}4i.3. Redirects the user to the login page.
				\item[5i.] Critical Service Outage. \\
				\hspace{1cm}5i.1. System detects that a microservice is down. \\
				\hspace{1cm}5i.2. System displays a message indicating the metric information is unavailable and not up-to-date. \\
				\hspace{1cm}5i.3. System continues to serve cached data for other functional metrics.
			\end{itemize}
		\item Public User \\
			N/A
		\item Human Resources \\
		...
		\end{itemize}
		\begin{enumerate}[1.]
			\item External User accesses the \textbf{SCEMAS} application home screen.
			\item System retrieves the user's last-used geographical area and sets it as the current geographical location.
			\item System returns nearby currently active environmental and health warnings and displays them in a real-time dashboard map.
			\item External User clicks on a warning on the map to view more information.
			\item System opens a side panel showing all official information, including severity levels, specific metrics, and health precaution recommendations.
			\item External User views information and closes the side panel.
			\item System closes the side panel and returns to the dashboard map.
			\item External User selects a data type (air quality, noise levels, temperature, \textbf{UV} levels, and humidity) from the metric toggle menu.
			\item System updates the map to display the selected data type on the real-time map with colour-coded environmental maps based on the severity levels of nearby regions.
			\item External User clicks the toggle bar to select "View Alerts."
			\item System identifies the user's account permissions and loads all current environmental alerts and system logs to the dashboard.
			\item External User monitors active alerts and system logs for consistency.
			\item External User clicks log out.
			\item System invalidates the current session, clears cached map data, and returns the user to the login screen.
		\end{enumerate}
		\begin{itemize}
			\item[] {\bf Secondary Scenario:}
			...
			\hspace{1cm}2i.2. System retrieves the user's default home location from the user's account information. \\
			\hspace{1cm}2i.3. System sets the default area as the current location and loads the map. \\
			\hspace{1cm}2i.4. External User updates their geographical area by searching on the map.
			\item[3i.] No Critical Alerts Found. \\
			\hspace{1cm}3i.1. System returns no critical alerts nearby. \\
			\hspace{1cm}3i.2. System displays a "No Active Critical Alerts" message. \\
			\hspace{1cm}3i.3. System continues to display the base map and environmental metrics side panel.
			\item[4i.] Login Timed Out. \\
			\hspace{1cm}4i.1. System notices over 15 mins since the last user interaction. \\
			\hspace{1cm}4i.2. System clears all sensitive environmental data from the screen and clears any unsaved changes. \\
			\hspace{1cm}4i.3. System redirects the user to the login page.
			\item[5i.] Critical Service Outage. \\
			\hspace{1cm}5i.1. System detects that a microservice is down. \\
			\hspace{1cm}5i.2. System displays a message indicating the metric information is unavailable and not up-to-date. \\
			\hspace{1cm}5i.3. System continues to serve cached data for other functional metrics.
	...
	\item Public API Request to Access Data
		\begin{itemize}
			\item[] {\bf Pre-Condition:} The public user has read the API documentation. There is valid data flowing from the app to the API.
		\end{itemize}
		\begin{enumerate}[{\bf VP1.}]
		\item Administrator
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[3i.] Administrator can manually block an IP address. \\
				\hspace{1cm}3i.1. Admin can view past metrics from an IP address from the API request database to access if an IP needs to be blocked. \\
				\hspace{1cm}3i.2. Admin can block an IP address by blacklisting it.
			\end{itemize}
		\item City Operator \\
			N/A
		\item Public User
			\begin{itemize}
				\item[] {\bf Pre-Condition:} \textbf{SCEMAS} is online, the API portal is configured by the administrator and valid aggregated data is in the system.
				\item[] {\bf Main Success Scenario:}
			\end{itemize}
			\begin{enumerate}[1.]
				\item External user sends a HTTP request to the endpoint for environmental data.
				\item System receives HTTP request, and processes it by checking if the request is asking for valid data.
				\item System checks rate limiting counter from requesting IP address and the API request is counted in the database for metrics.
				\item System retrieves data requested from the database.
				\item System appends legal disclaimer to request.
				\item Systems sends HTTP response back to user.
				\item External user receives data.
			\end{enumerate}
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[2i.] HTTP request is not valid. \\
				\hspace{1cm}2i.1. System disregards HTTP request. \\
				\hspace{1cm}2i.2. System responds with not found HTTP response.
				\item[3i.] Rate limit exceeded. \\
				\hspace{1cm}3i.1. System drops the request and does not process it. \\
				\hspace{1cm}3i.2. System returns too many request HTTP response back to user.
			\end{itemize}
		\item Human Resources \\
			N/A
		\item Supply Chain \\
		...
		\item Legal
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[5i.] Disclaimer is provided in the HTTP response to protect the system and city from liability.
			\end{itemize}
		\end{enumerate}
		{\bf Global Scenario:}
		\begin{itemize}
			\item[] {\bf Pre-Condition:} \textbf{SCEMAS} is online and the API portal is configured by admin. Valid aggregated data is in the system.
			\item[] {\bf Main Success Scenario:}
		\end{itemize}
		\begin{enumerate}[1.]
			\item External user sends a HTTP request to the endpoint for environmental data.
		...
		\begin{itemize}
			...
			\item[3i.] Rate limit exceeded. \\
			\hspace{1cm}3i.1. System drops the request and does not process it. \\
			\hspace{1cm}3i.2. System returns too many HTTP requests response back to user.
		\end{itemize}

	\item Checking Audit log
		\begin{itemize}
			\item[] {\bf Pre-Condition:} \textbf{SCEMAS} system is online and actively logging activity in the audit log, Administrator has logged into the system.
		\end{itemize}
		\begin{enumerate}[{\bf VP1.}]
		\item Administrator
			\begin{itemize}
			...
			\end{itemize}
			\begin{enumerate}[1.]
				\item Administrator clicks on the "Audit Log" button.
				\item System re-verifies Administrator's permissions and grants them access to the audit logs.
				\item System displays the audit log page.
				\item Administrator clicks on the event tab they want to view records of (e.g. user management, alert triggers, device lifecycle changes).
				\item System retrieves relevant audit logs and displays them on the page.
				\item Administrator views audit logs of the specified event they were interested in.
			\end{enumerate}
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[2i.] System denies user access to admin controls. \\
				\hspace{1cm}2i.1. Administrator authentication is unsuccessful. \\
				\hspace{1cm}2i.2. Checking audit log fails.
				\item[5i.] No audit logs are present for the type of event being viewed. \\
				\hspace{1cm}5i.1. System presents a message indicating there are no audit logs for the given event type.
				\item[5ii.] System fails to retrieve audit logs. \\
				\hspace{1cm}5ii.1. System presents an error message to notify user of failure. \\
				\hspace{1cm}5ii.2. Checking audit log fails.
			\end{itemize}
		\item City Operator
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[5i.] City operator's activity will appear within audit logs retrieved by administrator.
			\end{itemize}
		\item Public User \\
			N/A
		\item Human Resources
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[6i.] Audit logs may be used to flag suspicious or inappropriate activities of city operators for reporting to HR.
			\end{itemize}
		\item Supply Chain
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[6i.] Audit logs may be passed on to the supply chain to monitor the health of the system.
			\end{itemize}
		\item Legal
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[6i.] Audit logs may be passed on to the legal department as documented evidence of all the system's activity if required.
			\end{itemize}
		\end{enumerate}
		{\bf Global Scenario:}
		\begin{itemize}
	...
	\item User authentication
		\begin{itemize}
			\item[] {\bf Pre-Condition:} The user is knowledgeable with the system. There is valid data flowing into the system from various \textbf{IoT} devices.
		\end{itemize}
		\begin{enumerate}[{\bf VP1.}]
		\item Administrator
			\begin{itemize}
				\item[4i.2.] The administrator is prompted to review suspicious login.
				\item[4i.3.] The administrator is contacted by the user to help with the login issue.
			\end{itemize}
		\item City Operator
			\begin{itemize}
				\item[] {\bf Main Success Scenario:}
			\end{itemize}
			\begin{enumerate}[1.]
				\item User opens up the \textbf{SCEMAS} webapp in their browser.
				\item System requires the user to log in and displays the username and password fields.
				\item User enters username and password.
				\item System authenticates the user.
				\item System loads the dashboard and displays all options.
			\end{enumerate}
			\begin{itemize}
				\item[] {\bf Secondary Scenario:}
				\item[4i.] System fails to authenticate the user. \\
				\hspace{1cm}4i.1. System displays login error on screen. \\
				\hspace{1cm}4i.2. System prompts the administrator to review suspicious log in. \\
				\hspace{1cm}4i.3. System asks if the user wants to contact the administrator for help.
			\end{itemize}
		\item Public User \\
			N/A
		\item Human Resources \\
		...
		\item Legal
			\begin{itemize}
				\item[11i.] If a public facing alert has been created the system checks if the alert is permitted within the law.
			\end{itemize}
		\end{enumerate}
		{\bf Global Scenario:}
		\begin{itemize}
		...
		\begin{itemize}
			...
			\item[4i.] System fails to authenticate the user. \\
			\hspace{1cm}4i.1. System displays login error on screen. \\
			\hspace{1cm}4i.2. System prompts the administrator to review suspicious login. \\
			\hspace{1cm}4i.3. System asks if the user wants to contact the administrator for help.
		\end{itemize}
\end{enumerate}
...
\begin{enumerate}[{LF-A}1. ]
	...
	\item All text in the application should be at least \textbf{14-px} in size.\newline
...
\begin{enumerate}[{LF-S}1. ]
	...
	\item The application's interface must utilize modern conventions of professional software \textbf{UI/UX} to achieve a professional and minimalist design.\newline
	{\bf Rationale:} Adhering to commonly used \textbf{UI/UX} design principles will make the application look polished and professional, instilling trust in the user. Using well-known conventions of web design will also help the application to be more user-friendly [3].
...
\begin{enumerate}[{UH-L}1. ]
	...
	{\bf Rationale:} A short learning curve ensures the API is intuitive and well-documented for the public. A short onboarding period encourages adoption of the \textbf{REST API} and improves developer productivity [4].
\end{enumerate}
% End SubSubSection
...
\begin{enumerate}[{PR-RA}1. ]
	...
	{\bf Rationale:} Disaster recovery aims to minimize business disruption by restoring service quickly after failures. This requirement sets an explicit \textbf{RTO} to reduce the operational "blind spot" during time-sensitive environmental incidents [12].
\end{enumerate}
% End SubSubSection
...
\begin{enumerate}[{PR-RFT}1. ]
	...
	{\bf Rationale:} Smart-city sensor networks must tolerate intermittent connectivity, including extended outages during major emergencies. \textbf{IoT} architectures commonly support offline operation by using local caching so telemetry is not lost and can be reliably forwarded when connectivity returns [13].
...
\begin{enumerate}[{OE-IA}1. ]
	\item The system must implement a read-only \textbf{REST API} endpoint allowing third-party and public access to non-sensitive aggregated environmental data.\newline
...
\begin{enumerate}[{OE-P}1. ]
	...
	{\bf Rationale:} {\color{red} The application should be able to install in under 10 minutes to ensure a frictionless installation experience for users [4].}
\end{enumerate}
% End SubSubSection
...
\begin{enumerate}[{SR-P}1. ]
	\item The system shall not collect, process, or store any personally identifiable information (\textbf{PII}).\newline
...
\begin{enumerate}[{LR-COMP}1. ]
	\item The app will not process any raw audio data. \textbf{IoT} devices will process audio and never transmit raw audio streams only sound levels.\newline
...
% Begin Section
The chosen innovative feature we will be implementing is a public interface for our \textbf{SCEMAS} application that will allow any public user to view dashboards which will visualize all of the publicly available environmental metrics available from the \textbf{REST API}. This feature will benefit the product by increasing the number of users for our application. Since the \textbf{REST API} will be mainly used by third-party developers, we want to allow regular public users to also view and be informed of environmental issues. We have also come up with other innovative features, as listed below:
...
\begin{itemize}
	...
	\item Use-case Diagram as a group
	\item BE3. Sunlight is very high on the \textbf{UV} index
